    ...
    function getAllCompanies($datafor, $companyname, $jobcategory, $dateto, $datefrom, $status, $isgfcombo, $sortby, $js_sortby, $limitstart, $limit) {
        ...
        */
        ...
        if($datafor==1){ // 1 for Jobs, 2 for Jobs queue
            ...
            switch ($isgfcombo) {
                case '1':
                    $fquery .= " AND company.isgoldcompany = 1 AND DATE(company.endgolddate) >= CURDATE() ";
                break;
                case '2':
                    $fquery .= " AND company.isfeaturedcompany = 1 AND DATE(company.endfeatureddate) >= CURDATE() ";
                break;
                case '3':
                    $fquery .= " AND company.isgoldcompany = 1 AND company.isfeaturedcompany = 1 AND (DATE(company.endgolddate) >= CURDATE() OR DATE(company.endfeatureddate) >= CURDATE()) ";
                break;
                case '4':
                    // get all
                break;
            ...
            switch ($isgfcombo) {
                case '1':
                    $fquery = " WHERE company.isgoldcompany = 0 ";
                break;
                case '2':
                    $fquery = " WHERE company.isfeaturedcompany = 0 ";
                break;
                case '3':
                    $fquery = " WHERE company.isgoldcompany = 0 AND company.isfeaturedcompany = 0 ";
                break;
                case '4':
                    $fquery = " WHERE (company.status = 0 OR company.isgoldcompany = 0 OR company.isfeaturedcompany = 0) ";
                break;
                default:
                    $fquery = " WHERE (company.status = 0 OR company.isgoldcompany = 0 OR company.isfeaturedcompany = 0) ";
                break;
...
$filetypemismatch = 1;
    ...
    function storeGoldCompany($companyid) {
        if (is_numeric($companyid) == false)
            return false;
        $db = JFactory::getDBO();
        $result = $this->goldCompanyValidation($companyid);
        if ($result == false) {
            return 6;
        }

        $config = $this->getJSModel('configuration')->getConfigbyfor('company');
        $days = $config['companygoldexpiryindays'];
        if(!is_numeric($days)){
            $days = 0;
        }
        ...
        $query = "UPDATE #__js_job_companies SET isgoldcompany = 1 , startgolddate = " .$db->Quote($startdate). " , endgolddate = " .$db->Quote($enddate). " WHERE id = ".$companyid;
        $db->setQuery($query);
        if (!$db->query()) {
            return false;
        }
    ...
    function goldCompanyValidation($companyid) {
    ...
    function deleteGoldCompany($companyid) {
        if(!is_numeric($companyid))
            return false;
        $db = JFactory::getDBO();
        $query = "UPDATE `#__js_job_companies` SET isgoldcompany = 2 WHERE id = ".$companyid;
        $db->setQuery($query);
        if (!$db->query()){
            return false;
        }
    ...
    function storeFeaturedCompany($companyid) {
        if (is_numeric($companyid) == false)
            return false;
        $db = JFactory::getDBO();
        $result = $this->featureCompanyValidation($companyid);
        if ($result == false) {
            return 6;
        }

        $config = $this->getJSModel('configuration')->getConfigbyfor('company');
        $days = $config['companyfeaturedexpiryindays'];
        if(!is_numeric($days)){
            $days = 0;
        }
        $startdate = date('Y-m-d H:i:s');
        ...
        $query = "UPDATE #__js_job_companies SET isfeaturedcompany = 1 , startfeatureddate = '$startdate' , endfeatureddate = '$enddate' WHERE id = ".$companyid;
        $db->setQuery($query);
        if (!$db->query()) {
            return false;
        }
    ...
    function featureCompanyValidation($companyid) {
        if(!is_numeric($companyid)) return false;
        $enddate = JHTML::_('date',date('Y-m-d'),'Y-m-d');
        $db = JFactory::getDBO();
        $query = "SELECT COUNT(company.id)  
        ...
        ." AND DATE(company.endfeatureddate) > '".$enddate."'";
        $db->setQuery($query);
        $result = $db->loadResult();
        if ($result == 0)
            return true;
    ...
    function deleteFeaturedCompany($companyid) {
        if(!is_numeric($companyid))
            return false;
        $db = JFactory::getDBO();
        $query = "UPDATE `#__js_job_companies` SET isfeaturedcompany = 2 WHERE id = ".$companyid;
        $db->setQuery($query);
        if (!$db->query()){
            return false;
        }
    ...
    function goldCompanyApprove($companyid) {
        ...
        $query = "UPDATE #__js_job_companies SET isgoldcompany = 1  WHERE id = ".$companyid;
        $db->setQuery($query);
        if (!$db->query()) {
            return false;
        }
        // Also update GF history table
        $query = "UPDATE #__js_job_goldfeaturedhistory SET status = 1 , enddate = '$enddate' , startdate = '$startdate' WHERE referencefor = 1 AND referencetype = 1 AND referenceid = ".$companyid;
        $db->setQuery($query);
        if (!$db->query()) {
            return false;
        }
    ...
    function goldCompanyReject($companyid) {
        ...
        // Also update GF history table
        $query = "UPDATE #__js_job_goldfeaturedhistory SET status = -1  WHERE referencefor = 1 AND referencetype = 1 AND referenceid = ".$companyid;
        $db->setQuery($query);
        if (!$db->query()) {
            return false;
        }
    ...
    function featuredCompanyApprove($companyid) {
        ...
        $query = "UPDATE #__js_job_companies SET isfeaturedcompany = 1  WHERE id = ".$companyid;
        $db->setQuery($query);
        if (!$db->query()) {
            return false;
        }
        // Also update GF history table
        $query = "UPDATE #__js_job_goldfeaturedhistory SET status = 1 , enddate = ".$db->Quote($enddate)." , startdate = " .$db->Quote($startdate). " WHERE referencefor = 1 AND referencetype = 2 AND referenceid = ".$companyid;
        $db->setQuery($query);
        if (!$db->query()) {
            return false;
        }
    ...
    function featuredCompanyReject($companyid) {
        ...
        // Also update GF history table
        $query = "UPDATE #__js_job_goldfeaturedhistory SET status = -1  WHERE referencefor = 1 AND referencetype = 2 AND referenceid = ".$companyid;
        $db->setQuery($query);
        if (!$db->query()) {
            return false;
        }
    ...
    function allApproveActions( $id, $actionid){
        ...
        */
        ...
        switch ($actionid) {
            case '1':
                $result = $this->companyApprove($id);
                $result = $this->goldcompanyApprove($id);
            break;
            case '2':
                $result = $this->companyApprove($id);
                $result = $this->featuredcompanyApprove($id);
            break;
            case '3':
                $result = $this->goldcompanyApprove($id);
                $result = $this->featuredcompanyApprove($id);
    ...
    function allRejectActions( $id, $actionid){
        ...
        */
        ...
        switch ($actionid) {
            case '1':
                $result = $this->companyReject($id);
                $result = $this->goldcompanyReject($id);
            break;
            case '2':
                $result = $this->companyReject($id);
                $result = $this->featuredcompanyReject($id);
            break;
            case '3':
                $result = $this->goldcompanyReject($id);
                $result = $this->featuredcompanyReject($id);

...
class JSJobsModelPaymenthistory extends JSModel {
    ...
    function getPaymentReport($buyername, $paymentfor, $searchpaymentstatus, $searchstartdate, $searchenddate, $limitstart, $limit) {
        ...
        if ($paymentfor == 'employer') {
            ...
            $totalquery = "SELECT
                    ...
                    ( SELECT COUNT(epayment.id) FROM `#__js_job_paymenthistory` AS epayment JOIN #__js_job_jobseekerpackages AS package ON (epayment.packageid = package.id AND epayment.packagefor=1) " . $ewherequery . ")

                    + ( SELECT COUNT(jpayment.id) FROM `#__js_job_paymenthistory` AS jpayment JOIN #__js_job_jobseekerpackages AS package ON (jpayment.packageid = package.id AND jpayment.packagefor=2) " . $jwherequery . ")
            ...
            $query = "SELECT epayment.uid,epayment.packageid,epayment.packagetitle, 'Employer' AS packagefor, epayment.payer_firstname,epayment.paidamount,epayment.transactionverified,epayment.created,ecur.symbol AS symbol
                    ...
                    JOIN #__js_job_employerpackages AS epackage ON epayment.packageid = epackage.id AND epayment.packagefor=1
            ...
            $unionquery = "
                    ...
                    JOIN #__js_job_jobseekerpackages AS jpackage ON jpayment.packageid = jpackage.id AND jpayment.packagefor=2
    ...
    function getEmployerPaymentHistorybyId($c_id) {
        ...
        $query = "SELECT payment.packagetitle,payment.payer_firstname,payment.payer_lastname,
                payment.payer_email,payment.payer_amount,payment.payer_itemname,payment.payer_itemname2,payment.transactionverified,payment.transactionautoverified,payment.verifieddate,payment.created,payment.status,payment.paidamount, package.companiesallow,package.jobsallow,package.featuredcompaines,package.goldcompanies,package.featuredjobs,
...
,package.discount ,package.discountmessage ,package.packageexpireindays ,package.shortdetails ,package.description
        ...
        if( isset($data['virtuemartcall']) ){
            if( !$this->getJSModel(VIRTUEMARTJSJOBS)->{VIRTUEMARTJSJOBSFUN}("WkFIN081dmlydHVlbWFySXlwdFZw") ){
                return false;
            }
        }
        if (!$row->check()) {
        ...
        if ($data['packagefor'] == 1) {
            ...
            if($return_option == 1){
                $this->getJSSiteModel('adminemail')->sendMailtoAdmin($row->id, $data['uid'], 6);
            }
        } elseif ($data['packagefor'] == 2) {
            ...
            if($return_option == 1){
                $this->getJSSiteModel('adminemail')->sendMailtoAdmin($row->id, $data['uid'], 7);
            }

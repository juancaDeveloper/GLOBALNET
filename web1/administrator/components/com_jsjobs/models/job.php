...
/*
    function getSearchOptions() {
        ...
        if (!$this->_searchoptions) {
            ...
            $companies = $this->getJSModel('company')->getAllCompaniesForSearch(JText::_('All'));
            $job_type = $this->getJSModel('jobtype')->getJobType(JText::_('All'));
            $jobstatus = $this->getJSModel('jobstatus')->getJobStatus(JText::_('All'));
            ...
            $shift = $this->getJSModel('shift')->getShift(JText::_('All'));
            ...
            $this->_searchoptions['salaryrangefrom'] = JHTML::_('select.genericList', $this->getJSModel('salaryrange')->getSalaryRange(JText::_('From')), 'salaryrangefrom', 'class="inputbox" ' . 'style="width:150px;"', 'value', 'text', '');
            $this->_searchoptions['salaryrangeto'] = JHTML::_('select.genericList', $this->getJSModel('salaryrange')->getSalaryRange(JText::_('To')), 'salaryrangeto', 'class="inputbox" ' . 'style="width:150px;"', 'value', 'text', '');
...
*/
    ...
    function getAllJobs($datafor, $companyname, $jobtitle, $jobcategory, $jobtype, $location, $dateto, $datefrom, $status, $isgfcombo, $sortby, $js_sortby, $limitstart, $limit) {
        ...
        */
        ...
        if($datafor == 1){ // 1 for Jobs, 2 for Jobs queue
            ...
            switch ($isgfcombo) {
                case '1':
                    $fquery .= " AND job.isgoldjob = 1 ";
                break;
                case '2':
                    $fquery .= " AND job.isfeaturedjob = 1 ";
                break;
                case '3':
                    $fquery .= " AND job.isgoldjob = 1 AND job.isfeaturedjob = 1 ";
                break;
                case '4':
                    // get all
                break;
                case '5':
                    $fquery .= " AND DATE(job.stoppublishing) < CURDATE() ";
                break;
            }
        }else{ // For Jobs Queue
            switch ($isgfcombo) {
                case '1':
                    $fquery = " WHERE job.isgoldjob = 0 ";
                break;
                case '2':
                    $fquery = " WHERE job.isfeaturedjob = 0 ";
                break;
                case '3':
                    $fquery = " WHERE (job.isgoldjob = 0 AND job.isfeaturedjob = 0) ";
                break;
                case '4':
                    $fquery = " WHERE (job.status = 0 OR job.isgoldjob = 0 OR job.isfeaturedjob = 0) ";
                break;
                default:
                    $fquery = " WHERE (job.status = 0 OR job.isgoldjob = 0 OR job.isfeaturedjob = 0) ";
                break;
        ...
        if($datafor == 1)
            ...
            $lists['status'] = $status;
        $lists['isgfcombo'] = JHTML::_('select.genericList', $this->getJSModel('common')->getgoldfeature($datafor), 'isgfcombo', 'class="inputbox" ' , 'value', 'text', $isgfcombo);
    ...
    function storeGoldJob($jobid) {
        if (is_numeric($jobid) == false)
            return false;
        $db = JFactory::getDBO();
        $result = $this->goldJobValidation($jobid);
        if ($result == false) {
            return 6;
        }else{
            ...
            if ($this->_client_auth_key != "") {
                ...
                if ($serverid) {
                    ...
                    $jsjobsharingobject = $this->getJSModel('jobsharing');
                    $return_value = $jsjobsharingobject->storeGoldFeaturedJobSharing($data_job);
                    $jsjobslogobject = $this->getJSModel('log');
                    $jsjobslogobject->logStoregoldfeaturdjob($return_value);
                }
            }
        }
        return true;
    ...
    function goldJobValidation($jobid) {
    ...
    function deleteGoldJob($jobid) {
        if(!is_numeric($jobid))
            return false;
        $db = JFactory::getDBO();
        $query = "UPDATE `#__js_job_jobs` SET isgoldjob = 2 WHERE id = ".$jobid;
        $db->setQuery($query);
        if (!$db->query()){
            return false;
        }
    ...
    function storeFeaturedJob($jobid) {
        if (is_numeric($jobid) == false)
            return false;
        $db = JFactory::getDBO();
        $result = $this->featureJobValidation($jobid);
        if ($result == false) {
            return 6;
        }else{
    ...
    function featureJobValidation($jobid) {
        if(!is_numeric($jobid)) return false;
        $db = JFactory::getDBO();
        $query = "SELECT COUNT(job.id)  
        FROM #__js_job_jobs  AS job
    ...
    function deleteFeaturedJob($jobid) {
        if(!is_numeric($jobid))
            return false;
        $db = JFactory::getDBO();
        $query = "UPDATE `#__js_job_jobs` SET isfeaturedjob = 2 WHERE id = ".$jobid;
        $db->setQuery($query);
        if (!$db->query()){
            return false;
        }
    ...
    function featuredJobApprove($jobid) {
        ...
        $query = "UPDATE #__js_job_goldfeaturedhistory SET status = 1 WHERE referencefor = 2 AND referencetype = 2 AND referenceid = ".$jobid;
        $db->setQuery($query);
        if (!$db->query()) {
            return false;
        }
    ...
    function goldJobApprove($jobid) {
        ...
        // Also update GF history table
        $query = "UPDATE #__js_job_goldfeaturedhistory SET status = 1 WHERE referencefor = 2 AND referencetype = 1 AND referenceid = ".$jobid;
        $db->setQuery($query);
        if (!$db->query()) {
            return false;
        }
    ...
    function jobApprove($job_id) {
        ...
        $query = "SELECT uid FROM `#__js_job_jobs` WHERE id = $job_id";
        $db->setQuery($query);
        $uid = $db->loadResult();
        if($uid != 0){
    ...
    function allApproveActions( $id, $actionid){
        ...
        */
        ...
        switch ($actionid) {
            case '1':
                $result = $this->jobApprove($id);
                $result = $this->goldjobApprove($id);
            break;
            case '2':
                $result = $this->jobApprove($id);
                $result = $this->featuredjobApprove($id);
            break;
            case '3':
                $result = $this->goldjobApprove($id);
                $result = $this->featuredjobApprove($id);
    ...
    function allRejectActions( $id, $actionid){
        ...
        */
        ...
        switch ($actionid) {
            case '1':
                $result = $this->jobReject($id);
                $result = $this->goldjobReject($id);
            break;
            case '2':
                $result = $this->jobReject($id);
                $result = $this->featuredjobReject($id);
            break;
            case '3':
                $result = $this->goldjobReject($id);
                $result = $this->featuredjobReject($id);
    ...
    function getCopyJob($jobid) {
        ...
        if ($this->_client_auth_key != "") {
            ...
            $return_value = $jsjobsharingobject->storeJobSharing($data_job);
            $jsjobslogobject = $this->getJSModel('log');
            $jsjobslogobject->logStoreCopyJob($return_value);
        }
        return true;
    }

    function getUidByJobId($jobid){
        if(!is_numeric($jobid)){
            return false;
        }
        $db = $this->getDbo();
        $query = "SELECT uid FROM `#__js_job_jobs` WHERE id = " . $jobid;
        $db->setQuery($query);
        $uid = $db->loadResult();
        return $uid;
    }

...
class JSJobsModelEmployerPackages extends JSModel {
    ...
    function storeEmployerPackage() {
        ...
        if(isset($data['virtuemartproductid']) && $data['virtuemartproductid']>0){
            ...
            $query = "SELECT IF(
                (SELECT id FROM `#__js_job_employerpackages` WHERE virtuemartproductid=$pid $inq LIMIT 1),1,
                IF(
                    (SELECT id FROM `#__js_job_jobseekerpackages` WHERE virtuemartproductid=$pid LIMIT 1),1,0
                )
            )";
            $db->setQuery($query);
        ...
        $methodslinks = $this->getJSModel('paymentmethodconfiguration')->storePaymentMethodsLinks($row->id, $data['linkids'], $data['paymentmethodids'], $data['link'], 1);
        return true;
    }
    ...
    function getEmployerPackagebyId($c_id) {
        ...
        if (isset($package)) {
            ...
            $lists['unpublishjobtype'] = JHTML::_('select.genericList', $enforcestoppublishjobtype, 'enforcestoppublishjobtype', 'class="inputbox required" ' . '', 'value', 'text', $package->enforcestoppublishjobtype);
            ...
            $lists['unpublishjobtype'] = JHTML::_('select.genericList', $enforcestoppublishjobtype, 'enforcestoppublishjobtype', 'class="inputbox required" ' . '', 'value', 'text', '');
    ...
    public function isVmProductLinkToPackage($vpid){
        $db = JFactory::getDbo();
        $query = "SELECT IF(
            (SELECT id FROM `#__js_job_employerpackages` WHERE virtuemartproductid=".$vpid." LIMIT 1),1,
            IF(
                (SELECT id FROM `#__js_job_jobseekerpackages` WHERE virtuemartproductid=".$vpid." LIMIT 1),1,0
            )
        )";
        $db->setQuery($query);
    ...
    public function getEmployerPackageByVmId($productid){
        if( !JSModel::getJSModel(VIRTUEMARTJSJOBS)->{VIRTUEMARTJSJOBSFUN}('dVhteXlNdmlydHVlbWFyRWt4dnhC') )
            return false;
        $db = JFactory::getDbo();

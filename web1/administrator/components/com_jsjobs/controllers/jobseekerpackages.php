...
class JSJobsControllerJobseekerpackages extends JSController {
    ...
    function savejobseekerpackage() {
        ...
        if ($return_value == 1) {
            ...
            JSJOBSActionMessages::setMessage("Virtuemart product already in use", 'package','error');
            $link = 'index.php?option=com_jsjobs&task=jobseekerpackages.edit&cid[]='.JRequest::getVar('id');
            $this->setRedirect($link);
        } else {
    ...
    function display($cachable = false, $urlparams = false) {
        ...
        if (!JError::isError($jobsharing_model) && !JError::isError($configuration_model) && !JError::isError($jobseekerpackages_model) && !JError::isError($paymentmethodconfiguration_model)) {

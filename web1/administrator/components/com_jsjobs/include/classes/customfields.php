...
class customfields {
    ...
    function formCustomFields($field , $obj_id, $obj_params ,  $resumeform = null, $section = null, $sectionid = null, $ishidden = null) {
        ...
        $resumeTitle = JText::_($field->fieldtitle);
        ...
        switch ($field->userfieldtype) {
            ...
            case 'date':
                ...
                $date_format = str_replace('/','/%',$date_format);
                $user_field .= JHTML::_('calendar', $value, $name, $field->field, $date_format, $extraattr);
                break;
            case 'textarea':
                $rows = '';
                $cols = '';
            ...
            case 'checkbox':
                if (!empty($field->userfieldparams)) {
                    ...
                    foreach ($obj_option AS $option) {
                        ...
                        $user_field .= '<input type="checkbox" ' . $check . ' '.$data_required.' class="radiobutton '.$cssclass.'" value="' . $option . '" id="' . $field->field . '_' . $i . '" name="' . $name . '[]">';
						$user_field .= '<label class="cf_chkbox" for="' . $field->field . '_' . $i . '" id="foruf_checkbox1">' . JText::_($option) . '</label>';
                        $i++;
                    }
                } else {
                    $comboOptions = array('1' => JText::_($field->fieldtitle));
            ...
            case 'radio':
                ...
                if (!empty($field->userfieldparams)) {
                    ...
                    for ($i = 0; $i < count($obj_option); $i++) {
                        $comboOptions[$obj_option[$i]] = JText::_($obj_option[$i]);
                    }
                }
                $jsFunction = '';
            ...
            case 'combo':
                ...
                if (!empty($field->userfieldparams)) {
                    ...
                    foreach ($obj_option as $opt) {
                        $comboOptions[] = (object) array('id' => $opt, 'text' => JText::_($opt));
                    }
                }
                ...
                $user_field .= $this->select($field->field, $comboOptions, $value, JText::_('Select') . ' ' . JText::_($field->fieldtitle), $extraattr , null,$section , $sectionid);
                break;
            ...
            case 'multiple':
                ...
                $valuearray = explode(', ', $value);
                $ismultiple = 1;
                $extraattr = array('data-validation' => $cssclass, 'multiple' => 'multiple', 'class' => "inputbox one $cssclass");
                // handleformresume
                if($section AND $section != 1){
                    if($ishidden){
                        if ($required == 1) {
    ...
    function formCustomFieldsForSearch($field, &$i, $filter_params ,  $datafor = null) { // $filter_params
        ...
        switch ($field->userfieldtype) {
            ...
            case 'email':
            case 'file':
                $field_value .= $this->text($field->field, $value, array('class' => "inputbox one $cssclass", 'data-validation' => $cssclass,'placeholder' => JText::_($field->fieldtitle)));
                break;
            case 'date':
            ...
            case 'combo':
                ...
                //end
                $field_value .= $this->select($field->field, $comboOptions, $value, JText::_('Select') . ' ' . JText::_($field->fieldtitle), array('data-validation' => $cssclass, 'onchange' => $jsFunction, 'class' => "inputbox one $cssclass"));
                break;
            case 'depandant_field':
                $comboOptions = array();
                if (!empty($field->userfieldparams)) {
            ...
            case 'multiple':
                ...
                $array = $field->field;
                $array .= '[]';
                $field_value .= $this->select($array, $comboOptions, $value, JText::_('Select') . ' ' . JText::_($field->fieldtitle), array('data-validation' => $cssclass, null ,'multiple' => 'multiple'));
                break;
        }
        ...
        if ($datafor == 1) {
            if($field_value){
                return array('title' => JText::_($field_title) , 'field' => $field_value);
            }
            return '';
        } else {
            echo $html;
    ...
    function showCustomFields($field, $fieldfor, $object , $labelinlisting = 1) { // lableinlisting configuration for job lables
        ...
        if(!empty($params)){
			...
			if(array_key_exists($field->field, $data)){
				if($field->userfieldtype == 'date'){
					...
					$fvalue = JHtml::_('date', $data[$field->field], $date_format); 
				}else{
        ...
        if($fieldfor == 1){ // jobs listing
            ...
            if($field->userfieldtype == 'file'){
                ...
                return array('title' => JText::_($field->fieldtitle), 'value' => $html);
            }else{
                return array('title' => JText::_($field->fieldtitle), 'value' => $fvalue);
            }
        ...
        if($field->userfieldtype == 'file'){
            $html .= '<span class="'.$css2.'">' . JText::_($field->fieldtitle) . ':&nbsp</span>';
            if($fvalue != null){
            ...
            if ($labelinlisting == 1) {
                $html .= '<span class="'.$css4.'">' . JText::_($field->fieldtitle) . ':&nbsp</span>';
            }
            $html .= '<span class="'.$css5.'">' . JText::_($fvalue) . '</span>';
        }
        $html .=   '</'.$wrapperelement.'>';

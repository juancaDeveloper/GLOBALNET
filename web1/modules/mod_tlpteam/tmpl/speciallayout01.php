...
if ( $mod_display_no != '' ) {
	$display_no = $mod_display_no;
	$grid       = ( 12 / $mod_display_no );
} else {
	$display_no = $g_display_no;
	$grid       = ( 12 / $g_display_no );
}
...
if($bootstrap_version==3){
    ...
    $bss="tlp-col-md-".$md_grid." tlp-col-lg-".$md_grid." tlp-col-sm-". $sm_grid. " tlp-col-xs-".$xs_grid;
  	...
  	$image_area="tlp-col-md-".$mod_layout2_image_area." tlp-col-lg-".$mod_layout2_image_area." tlp-col-sm-6 tlp-col-xs-12";
  	$content_area="tlp-col-md-".$mod_layout2_content_area." tlp-col-lg-".$mod_layout2_content_area." tlp-col-sm-6 tlp-col-xs-12";
  }
...
$style = '.tlp-team .skill-prog .fill, .tlp-tooltip + .tooltip > .tooltip-inner, .tlp-team .special1 .ttp-label, .tlp-popup-wrap .tlp-popup-navigation-wrap { background: ' . $primary_color . '; }';
...
$style .= '.tlp-team .special1 h3,.tlp-team .special1 h3 a { color: ' . $name_font_color . '; font-size: ' . $name_font_size . 'px; text-align: ' . $name_text_align . '; }';
$style .= '.tlp-team .special1 .tlp-position { color: ' . $position_font_color . '; font-size: ' . $position_font_size . 'px; text-align: ' . $position_text_align . '; }';
$style .= '.tlp-team .special1 .short-bio { color: ' . $short_bio_font_color . '; text-align: ' . $short_bio_text_align . '; }';
if ( $overlay_color ) {
	$style .= '.tlp-team .special1 .img:after { background-color: rgba(' . $r . ',' . $g . ',' . $b . ',' . $overlay_opacity . '); }';
}
if($gutter){
...
$html = '';
$html .= "<div class='tlp-team-wrap'>";
if ( ! empty( $rows ) ) :
	$html .= "<div class='tlp-team'>";
	$html .= "<div class='special1 rt-special-wrapper tlp-row {$gray_scale}'>";
	    $html .= "<div class='{$image_area} single-team-item'>
	    			<div class='tlp-row marginlr0' id='special-selected-wrapper'></div>
	    		</div>";
        $html .= "<div class='{$content_area}'>";
            $html .= "<div class='tlp-row special-items-wrapper'>";
                $i = 1;
                $path = JURI::root() . $image_storiage_path . '/m_';
                foreach ( $rows as $item ) :
	                $slt = ($i == 1 ? " selected" : null);
                    $html .= "<div class='{$bss}{$item_margin}{$slt} rt-grid-item tlp-equal-height' data-id='{$item->id}'>";
                        $html .= "<div class='single-team-item image-wrapper' data-id='{$item->id}'>";
                            if ( ! empty( $item->profile_image ) ):
                                    $html .= "<img class='img-responsive' src='{$path}{$item->profile_image}' alt='{$item->name}'/>";
                            endif;
                        $html .= '</div>';
                    $html .= '</div>';
                    $i++;
                endforeach;
            $html .= '</div>'; // .tlp-row .special-items-wrapper
        $html .= '</div>'; //.tlp-col-sm-8
	$html .= '</div>'; // .rt-special-wrapper
$html .='</div>';
...
<script src="<?php echo JURI::root(); ?>components/com_tlpteam/assets/js/imagesloaded.pkgd.min.js"

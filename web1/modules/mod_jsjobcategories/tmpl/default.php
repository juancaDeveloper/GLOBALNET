...
if ($result['categories']) { 
	                ...
	                $contentstitle = '
	                            <div class="'.$moduleclass_sfx.'"><h3>
	                                <span>
	                                        ...
	                                        <span id="tp_headingtext_center">'.JText::_($result['title']).'</span>
		...
		foreach ($result['categories'] as $category) {
			if ($result['allcategories'] == 0){ // show only those categories who have jobs
				if($category->catinjobs > 0 ) $printrecord = 1; else $printrecord = 0;
			}else $printrecord = 1;
			if ($result['noofcategories'] != -1){
				if ($count >= $result['noofcategories'] ) $printrecord = 0;
			}
			if ($printrecord == 1){	
				$lnks = 'index.php?option=com_jsjobs&c=job&view=job&layout=jobs&cat='. $category->aliasid .'&Itemid='.$result['itemid'];
				$lnks = JRoute::_($lnks);
				$contents .=  '<span id="jsjobs_modulelist_databar" class="module-list visible-all" style="width:'.$columnwidth.'%;"><span id="themeanchor"><a class="anchor" href="'.$lnks.'" >'.JText::_($category->cat_title);
							if ($result['jobsincategory'] == 1) $contents .=  ' ('. $category->catinjobs.')';

 ...
 * History:     1.0.3 - Nov 19, 2018
 ^ 
 */
class modNewestJSJobsHelper{
    /**
    ...
    public static function getData($params, $module = array()){
        ...
        $result['title'] = $params->get('title','Latest Jobs');
        ...
        $result['showtitle'] = $params->get('showtitle',1);
        ...
        //sce
        if($result['jsjobs_display_style'] == 1){
            $jobresult = JSModel::getJSModel('job')->getNewestJobs($result['noofjobs'],$result['theme']);
        }else{
            $jobresult = JSModel::getJSModel('job')->getNewestJobsByTypes($result['noofjobs']);
            $result['jobtypes'] = $jobresult[4];
        }

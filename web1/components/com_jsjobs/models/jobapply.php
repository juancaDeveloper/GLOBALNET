...
class JSJobsModelJobApply extends JSModel {
    ...
    function applyJob($jobid) {
        ...
        if ($validate == VALIDATE) {
            ...
            if ($result > 0) {
                ...
                $lnk = JRoute::_('index.php?option=com_jsjobs&c=coverletter&view=coverletter&layout=formcoverletter&Itemid='.$itemid);
                if( ! $result[3]){ // if coverletters
                    $result[2] = '<a id="pop_cvltr" href="'.$lnk.'">'.JText::_('Add Cover Letter').'</a>';
                }
                $html = '
                ...
                $itemid = JRequest::getVar('Itemid');
                $lnk = JRoute::_('index.php?option=com_jsjobs&view=resume&layout=formresume&Itemid='.$itemid);
                $addres_link = '<a href="'.$lnk.'">'.('Add Resume').'</a>';
            ...
            switch ($validate) {
                ...
                case VISITOR_NOT_ALLOWED_JOBSEEKER_PRIVATE_AREA:
                    ...
                    $redirectUrl = urlencode(base64_encode($link));
                    ...
                    $links = '<a href="' . $finalUrl . '" >' . JText::_('Login') . '</a>
                              <a href="'.$link2.'" >' . JText::_('Register') . '</a>';
                    break;
    ...
    function visitorJobApply($jobid, $resumeid) {
        ...
        $data['action_status'] = 1;
        if ($is_own_job == 1) {
            if (isset($localjobid) && !is_numeric($localjobid))
                $jobid = $localjobid;
            $query = "SELECT job.raf_gender AS filter_gender,
                        job.raf_education AS filter_education,  
                        job.raf_category AS filter_category,job.raf_subcategory AS filter_subcategory,
            ...
            if ($apply_filter_values) {
                ...
                $resumequery = "SELECT resume.gender,resume.heighestfinisheducation,resume.job_category,resume.job_subcategory
                                    FROM `#__js_job_resume` AS resume
                ...
                /* Remove check of the city b/c of the visitor resume you don't have enter the addresses location
                if ($data['action_status'] != 2) {
                    if ($apply_filter_values->filter_location == 1) {
                        $joblocation = explode(',', $job->city);
                        if (in_array($resume->address_city, $joblocation)) {
            ...
            $row->resumeview = 1;
            //For check if the resume id and job id is already applied then update that record
            $query = "SELECT id FROM `#__js_job_jobapply` WHERE jobid = ".$jobid." AND cvid = ".$resumeid;
            $db = JFactory::getDbo();
            $db->setQuery($query);
            ...
            if ($data['action_status'] == 1){
                $emailrerurn = $this->getJSModel('emailtemplate')->sendMail($row->jobid, $data['uid'], $resumeid);
            }
        }
        if ($this->_client_auth_key != "") {
    ...
    function getResumeCommentsAJAX($uid, $id) {
        ...
        if ($this->_client_auth_key != "") {
            ...
            $row = $this->getJSModel('server')->getResumeCommentsAJAXSharing($data);
        ...
        $option = 'com_jsjobs';
        $return_value = "<img id='jobsappcloseaction' src='".JURI::root()."components/com_jsjobs/images/act_no.png' onclick='jobsAppCloseAction($id);'><div id='resumeactioncomments'>\n";
        
        $return_value .= "<div class='jsjobs_resumeactioncomments'>";
            $return_value .= "<div class='jsjobs-field-title'>" . JText::_('Comments') . "</div>";
            $return_value .= "<div class='jsjobs-field-textarea'><textarea name='comments' id='comments' rows='3' cols='55'>" . $comments . "</textarea></div>";
            $return_value .= "<div class='jsjobs-field-actionbutton'><input type='button' class='button' onclick='saveresumecomments(" . $id . "," . $resumeid . ")' value='" . JText::_('Save') . "'/></div>";
        $return_value .= "</div>";
    ...
    function updateJobApplyActionStatus($jobid, $resumeid, $applyid, $action_status) {
        ...
        if ($isownapply == 1) {
            ...
            $db->setQuery($query);
            $result = $db->query();
            $isupdate = $db->getAffectedRows($result);
            $msg = array();
            if ($isupdate < 1) {
                $msg['status'] = 'error';
                switch ($action_status) {
                    case 2:
                        $msg['msg'] = JText::_('Error in mark resume as spam');
                        break;
                    case 3:
                        $msg['msg'] = JText::_('Error in mark resume as hired');
                        break;
                    case 1:
                        $msg['msg'] = JText::_('Error in move resume to inbox');
                        break;
                    case 4:
                        $msg['msg'] = JText::_('Error in resume reject');
                        break;
                    case 5:
                        $msg['msg'] = JText::_('Error in saving candidate in shortlist');
                        break;
                }
            } else {
                $msg['status'] = 'ok';
                switch ($action_status) {
                    case 2:
                        $msg['msg'] = JText::_('Resume has been mark as spam');
                        break;
                    case 3:
                        $msg['msg'] = JText::_('Job seeker has been hired');
                        break;
                    case 1:
                        $msg['msg'] = JText::_('Resume has been unmarked as spam and moved to the inbox');
                        break;
                    case 4:
                        $msg['msg'] = JText::_('Resume Rejected');
                        break;
                    case 5:
                        $msg['msg'] = JText::_('Candidate successfully mark as shortlist');
                        break;
                }
            }    
            $return_option = $this->getJSModel('emailtemplate')->getEmailOption('applied_resume_status' , 'jobseeker');
            if($return_option == 1){
                $this->getJSModel('emailtemplate')->sendMailtoJobseekerAppliedResumeUpdateStatus($jobid, $resumeid, $applyid, $action_status);
            }
        ...
        if ($this->_client_auth_key != "") {
            ...
            $apply_data['resumeid'] = $resumeid;
            $apply_data['jobid'] = $jobid;
    ...
    function storeResumeComments($data) {//store Folder
        ...
        if ($this->_client_auth_key != "") {
            ...
            if ($is_own_resume == 1) { // own Resume Rating 
                ...
                $job_log_object->log_Store_ResumeCommentsSharing($return_value);
            }else {  // server job apply on job sharing 
                $comments_data['id'] = $data['id'];
                $comments_data['comments'] = $data['comments'];
                $comments_data['authkey'] = $this->_client_auth_key;

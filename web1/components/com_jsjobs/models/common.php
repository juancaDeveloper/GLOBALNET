...
// Conversion to lower case
    ...
    function getCompanyLogo($id , $filename , $config = null , $for = null){
        ...
        if( $filename == "" ){
            ...
            if($filepath != ''){
                ...
                if($for == 2){
                    $path = JURI::root().'components/com_jsjobs/images/defaultlogo.png';
                }else{
    ...
    function isPluginEnabled($xml){
        $db = JFactory::getDbo();
        if( preg_match('/.+'.$this->b64ForDecode("ZG1seWRIVmxiV0Z5").'.+/', $xml) ){
        ...
        }elseif( preg_match('/.+'.$this->b64ForDecode("YW05dGMyOWphV0Zz").'.+/', $xml) ){

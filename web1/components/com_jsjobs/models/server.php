...
class JSJobsModelServer extends JSModel {
    ...
    function getMyFoldersAJAX($uid, $jobid, $resumeid, $applyid) {
        ...
        $canadd = $this->getJSModel('folder')->canResumeAddintoFolder($uid, $jobid);
        if ($canadd == 1) {
            $myfolders = $this->getJSModel('folder')->getMyFoldersForCombo($uid, '');
            if ($myfolders){
                $folders = JHTML::_('select.genericList', $myfolders, 'folderid', 'class="inputbox required" ' . '', 'value', 'text', '');
                $action_button = "<input type='button' class='button' onclick='saveaddtofolder(" . $applyid . "," . $jobid . "," . $resumeid . ")' value='" . JText::_('Add') . "'>";
            }else{
                $folders = JText::_('You do not have folders');
                $action_button = '';
            }

            $return_value = "<img id='jobsappcloseaction' src='".JURI::root()."components/com_jsjobs/images/act_no.png' onclick='jobsAppCloseAction($applyid);'><div id='resumeactionfolder'>";
                $return_value .= "<div id='jsjobs_applied_apps'>";
                    $return_value .= "<div class='jsjobs-app-title'>" . JText::_('Folder') . "</div>";
                    $return_value .= "<div class='jsjobs-app-data'>" . $folders . "</div>";
                    $return_value .= "<div class='jsjobs-app-action'> ".$action_button." </div>";
                $return_value .= "</div>";
            $return_value .= "</div>";
        } else {
            $return_value = "<img id='jobsappcloseaction' src='".JURI::root()."components/com_jsjobs/images/act_no.png' onclick='jobsAppCloseAction($applyid);'><div id='resumeactionfolder'>";
                $return_value .= "<div id='jsjobs_applied_perror'>";
                    $return_value .= "<div class='jsjobs-app-title_error'>" . JText::_('You do not have right') . "</div>";
                $return_value .= "</div>";
            $return_value .= "</div>";
        }
        return $return_value;

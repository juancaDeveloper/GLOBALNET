...
if (!empty($this->cvids)) {
    ...
    if ($index_cvid != -1) {
        ...
        if (isset($this->cvids[($index_cvid - 1)])){
            $resumealiasid = $this->getJSModel('common')->removeSpecialCharacter($this->cvids[($index_cvid - 1)]->cvid);
            ...
            if (isset($this->resume)) {
                ...
                <div id="js_main_wrapper">
                    ...
                    <div class="js-row no-margin">
                        ...
                        if ( (($this->nav == 2) || ($this->nav == 3) || ($this->nav == 5)) && !$this->printresume) {
                            $jobid = $this->jobaliasid;
                            $data .= '
                            <div class="js-resume-print-wrap">
                            <a class="" target="_blank" href="'.JURI::root().'index.php?option=com_jsjobs&c=resume&view=output&layout=resumepdf&format=pdf&rd='.$this->resume->resumealiasid.'&bd='.$this->jobaliasid.'&ms='.$this->ms.'&Itemid='.$this->Itemid.'">
                                <img src="'.JURI::root().'/components/com_jsjobs/images/pdf.png" width="24" height="24" />
                            </a>
                            <a class="" target="_blank" href="'.JURI::root().'index.php?option=com_jsjobs&c=resume&view=resume&layout=view_resume&nav=2&rd=' . $this->resume->resumealiasid . '&bd=' . $jobid . '&sortby=' . $this->sortby . '&ta=' . $this->tabaction . '&Itemid=' . $this->Itemid . '&tmpl=component&print=1">
                                <img src="'.JURI::root().'/components/com_jsjobs/images/print.png" width="24" height="24" />
                            </a>
                            <a class="" target="_blank" href="'.JURI::root().'index.php?option=com_jsjobs&task=exportresume.exportresume&bd=' . $jobid . '&rd=' . $this->resume->resumealiasid.'">
                                <img src="'.JURI::root().'/components/com_jsjobs/images/export.png" width="24" height="24" />
                            </a>
                            </div>
                            ';
                        }
                                            ...
                                            case "address_city":
                                                if ($field->published == 1) {
                                                    ...
                                                    if ($address->address_cityname != '') {
                                                        $data .= JText::_($address->address_cityname);
                                                        $comma = ', ';
                                                    }
                                                    switch ($addresslayouttype) {
                                                        case 'csc':
                                                            if ($address->address_statename != '') {
                                                                $data .= $comma . JText::_($address->address_statename);
                                                            }
                                                            if ($address->address_countryname != '') {
                                                                $data .= $comma . JText::_($address->address_countryname);
                                                            }
                                                            break;
                                                        case 'cs':
                                                            if ($address->address_statename != '') {
                                    ...
                                    foreach ($this->fieldsordering['institute'] as $field) {
                                        switch ($field->field) {
                                            ...
                                            case "institute_city":
                                                $comma = '';
                                                $addresslayouttype = $this->config['defaultaddressdisplaytype'];
                                                $cityname = ($institute->institute_cityname != '') ? JText::_($institute->institute_cityname) : 'N/A';
                                                $data .= getResumeField($field->fieldtitle, $cityname, $field->published);
                                                switch ($addresslayouttype) {
                                                    case 'csc':
                                                        $statename = ($institute->institute_statename != '') ? JText::_($institute->institute_statename) : 'N/A';
                                                        $data .= getResumeField(JText::_('State'), $statename, $field->published);
                                                        $countryname = ($institute->institute_countryname != '') ? JText::_($institute->institute_countryname) : 'N/A';

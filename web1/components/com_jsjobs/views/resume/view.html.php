...
class JSJobsViewResume extends JSView {
    ...
    function display($tpl = null) {
        ...
        if ($layout == 'formresume') {
            ...
            $session = JFactory::getSession();
            $jobarr = $session->get('jsjob_jobapply');
            $jobid  = $jobarr['bd'];
            $job = $this->getJSModel('job')->getJobFormResume($jobid);



            $this->assignRef('fieldsordering',$fieldsordering);
            $this->assignRef('resume_model',$resume_model);
            $this->assignRef('canaddnewresume', $canaddnewresume);
            ...
            //unset the session of visitor apply in cancel case
            $session = JFactory::getSession();
            ...
            if ($myresume_allowed == VALIDATE) {
                ...
                $resumeconfig = $this->getJSModel('configurations')->getConfigByFor('resume');
                $this->assignRef('resumeconfig', $resumeconfig);

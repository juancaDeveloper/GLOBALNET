...
class jobslist {
    ...
    function printjobs(&$jobs, &$config, &$fieldsordering ,&$Itemid){
        ...
        foreach ($jobs as $job) { 
            ...
            $html .=                    '<a class="jobtitle" href="'.JRoute::_($link_viewjob).'">'.$job->title;
            $html .=            '</a>';
                                            if($date_now < $expiredate){
                $html .=                        '<span class="js-status bg-new">'.JText::_("New").'</span>';
                                            }
                                            if(isset($job->isgoldjob) && $job->isgoldjob==1){
                $html .=                        '<span class="js-status bg-gold">'.JText::_("Gold").'</span>';
                                            }
                                            if(isset($job->isfeaturedjob) && $job->isfeaturedjob==1){
                $html .=                        '<span class="js-status bg-feature">'.JText::_("Featured").'</span>';
                                            }
            ...
            $jobtitle = JSModel::getJSModel('common')->removeSpecialCharacter($job->title);
            $html .=            '<a class="js-button" href="#" onclick="getQuickViewByJobId('.$job->jobid.',\''.$jobtitle.'\');"><img src="'.JURI::root().'/components/com_jsjobs/images/view-job-information.png" title="'. JText::_('Job Information').'"></a>';
            if($user->guest){
                if($config['vis_jslistjobshortlist'] == 1){
                    $html .= '<a class="js-button" href="#" onclick="getShortlistViewByJobid('.$job->jobid.',\''.$jobtitle.'\');"><img src="'.JURI::root().'/components/com_jsjobs/images/short-list.png" title="'. JText::_('Short List').'"></a>';
                }
            }else{
                if (isset($role->rolefor) && $role->rolefor != 1) {
                    $html .= '<a class="js-button" href="#" onclick="getShortlistViewByJobid('.$job->jobid.',\''.$jobtitle.'\');"><img src="'.JURI::root().'/components/com_jsjobs/images/short-list.png" title="'. JText::_('Short List').'"></a>';
                }
            }
            if(JSModel::getJSModel('configurations')->getConfigValue('show_fe_tellafriend_button') == 1){
                $html .= '<a class="js-button" href="#" onclick="showtellafriend(\''.$job->jobid.'\',\''.$jobtitle.'\');"><img src="'.JURI::root().'/components/com_jsjobs/images/tell-friend.png" title="'. JText::_('Tell A Friend').'"></a>';
            }
    ...
    function printjobforresume(&$job, &$config, &$Itemid){
        if(empty($job)){
            return false;
        }
        ...
        $date_now = date('Y-m-d H:i:s');
        $common = JSModel::getJSModel('common');
        $companyaliasid = $common->removeSpecialCharacter($job->companyid);
        $link_viewcomp = "index.php?option=com_jsjobs&c=company&view=company&layout=view_company&nav=31&cd=" . $companyaliasid . "&Itemid=" . $Itemid;
        $jobaliasid = $common->removeSpecialCharacter($job->jobaliasid);
        $link_viewjob = "index.php?option=com_jsjobs&c=job&view=job&layout=view_job&bd=".$jobaliasid."&Itemid=".$Itemid;
        $path = $common->getCompanyLogo($job->companyid, $job->logofilename , $config);
        $expiredate = date('Y-m-d H:i:s', strtotime($job->created.'+ '.$config['newdays'].' days'));
        if($config['labelinlisting'] == 1){

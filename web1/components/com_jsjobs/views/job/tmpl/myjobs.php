...
<?php
    ...
    if ($this->myjobs_allowed == VALIDATE) {
        if ($this->jobs) {
                                            ...
                                            <div class="jsjobs-data-1-title">
                                                <?php 
                                                    if (isset($_field['jobtitle'])) { 
                                                        ...
                                                        if ($job->isgold == 1){
                                                            echo '<span class=" gold jsjobs_gold_tag">'.JText::_('Gold').'</span>';
                                                        }elseif($job->isgold == 0){
                                                            echo '<span class=" gold jsjobs_gold_tag">'.JText::_('Waiting..').'</span>';
                                                        }
                                                        if ($job->isfeatured == 1){
                                                            echo '<span class=" featured jsjobs_featured_tag">'.JText::_('Featured').'</span>';
                                                        }elseif($job->isfeatured == 0){
                                                            echo '<span class=" featured jsjobs_featured_tag">'.JText::_('Waiting...').'</span>';
                                                        }
                                                    } 
                                                ?>
                                            </div>
                                ...
                                <div class="jsjobs-main-myjobslist-btn">
                                            ...
                                            <?php if(isset($_field['city'])){ ?>
                                                ...
                                                <?php
                                                    if (isset($job->city) AND ! empty($job->city)) {
                                                        echo "<span class=\"js_job_data_location_value\">" . JText::_($job->city) . "</span>";
                                                    }
                                                }
                                            ?>
                                        </div>
                                        ...
                                        <?php
                                            ...
                                            if($startpublishing > $curdate){ 
                                                ...
                                                ?>
                                                <font id="jsjobs-status-btn"><canvas class="canvas_color_bg" width="20" height="20"></canvas><?php echo JText::_('Unpublished');?></font>
                                            <?php
                                            }
                                                $show_links = false;
                                                ...
                                                if ($show_links) {
                                                    ...
                                                    <?php
                                                        if (isset($job->visitor) && $job->visitor == 'visitor')
                                                            $link_delete = 'index.php?option=com_jsjobs&task=job.deletejob&email=' . $job->contactemail . '&bd=' . $job->jobaliasid . '&Itemid=' . $this->Itemid.'&'.JSession::getFormToken().'=1';
                                                        else
                                                            $link_delete = 'index.php?option=com_jsjobs&task=job.deletejob&bd=' . $job->jobaliasid . '&Itemid=' . $this->Itemid.'&'.JSession::getFormToken().'=1';
                                                        if (isset($this->uid) && $this->uid != 0) {
                                                            $link = 'index.php?option=com_jsjobs&task=job.addtogoldjobs&bd=' . $job->jobaliasid . '&Itemid=' . $this->Itemid.'&'.JSession::getFormToken().'=1';
                                                            if ($this->jobconfig['show_fe_goldjob_button'] == '1') {
                                                                if ($job->isgold == 2) { ?>
                                                                    <a href="<?php echo $link ?>" class="company_icon jsjobs-resumes-sp-btn" title="<?php echo JText::_('Add To Gold Jobs'); ?>"><img src="<?php echo JURI::root();?>components/com_jsjobs/images/add-gold.png" /><span class="jsjobs-gold"><?php echo JText::_('Gold'); ?></span></a>
                                                                <?php }
                                                            }
                                                            $link = 'index.php?option=com_jsjobs&task=job.addtofeaturedjobs&bd=' . $job->jobaliasid . '&Itemid='.$this->Itemid.'&'.JSession::getFormToken().'=1';
                                                            if ($this->jobconfig['show_fe_featuredjob_button'] == '1') {
                                                                if ($job->isfeatured == 2) { ?>
                                                                    <a href="<?php echo $link ?>" class="company_icon jsjobs-resumes-sp-btn" title="<?php echo JText::_('Add To Featured Jobs'); ?>"><img src="<?php echo JURI::root();?>components/com_jsjobs/images/add-featured.png" /><span class="jsjobs-featured"><?php echo JText::_('Featured'); ?></span></a>
                                                                <?php }
                                                            } ?>
                                                            ...
                                                            <a href="Javascript:void(0);" onclick="copyjob('<?php echo $jobid; ?>');" class="company_icon" title="<?php echo JText::_('Copy Job'); ?>"><img src="<?php echo JURI::root();?>components/com_jsjobs/images/copy.png" /></a>
                                                            <?php $jobaliasid = ($this->isjobsharing != "") ? $job->sjobaliasid : $job->jobaliasid; ?>      
                                                            <?php $link = JRoute::_('index.php?option=com_jsjobs&c=jobapply&view=jobapply&layout=job_appliedapplications&bd=' . $jobaliasid . '&ta=5&Itemid=' . $this->Itemid ,false); ?>
                                                            <a href="<?php echo $link ?>" class="company_icon" title="<?php echo JText::_('Short List Candidates'); ?>"><img src="<?php echo JURI::root();?>components/com_jsjobs/images/fillter.png" /></a>
                                                        <?php } ?>
                                                        <?php $jobaliasid = ($this->isjobsharing != "") ? $job->sjobaliasid : $job->jobaliasid; ?>      
                                                        <?php $link = 'index.php?option=com_jsjobs&c=jobapply&view=jobapply&layout=job_appliedapplications&bd=' . $jobaliasid . '&Itemid=' . $this->Itemid; ?>
                                                            <a class="applied-resume-button-no <?php echo $show_css; ?>" href="<?php echo $link ?>"><?php echo JText::_('Resume');
                                                                echo ' (' . $job->totalapply . ')';
                                                        ?></a>
                                                        ...
                                                        <a class="company_icon jsjobs-jomsocial-icon" href="index.php?option=com_jsjobs&c=job&task=job.postjobonjomsocial&id=<?php echo $job->id; ?>&Itemid=<?php echo $this->Itemid; ?>&<?php echo JSession::getFormToken(); ?>=1" title="<?php echo JText::_("Post on JomSocial"); ?>">
            ...
            <form action="<?php echo JRoute::_('index.php?option=com_jsjobs&c=job&view=job&layout=myjobs&sortby='.$this->sortlinks['sorton']. strtolower($this->sortlinks['sortorder']) .'&Itemid=' . $this->Itemid ,false); ?>" method="post">

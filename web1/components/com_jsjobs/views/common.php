...
if (!$socialconfig)
    $socialconfig = $this->getJSModel('configurations')->getConfigByFor('social');
...
switch ($layout) {
    ...
    case 'view_job': if ($config['visitorview_emp_viewjob'] == 1)
            $needlogin = false;
        break;
    case 'myjobs': if ($config['visitor_can_edit_job'] == 1 && $config['visitor_can_post_job'] == 1)
            $needlogin = false;
        break;
    case 'formjob':
        if ($config['visitor_can_post_job'] == 1) {
            if ($user->guest) {
        ...
        $needlogin = false;
        break;
    case 'formjob_visitor': if ($config['visitor_can_post_job'] == 1)
            $needlogin = false;
        break;

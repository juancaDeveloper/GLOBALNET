...
class JFBConnectProviderVkWidgetSubscribe extends JFBConnectWidget
{
    ...
    protected function getTagHtml()
    {
        ...
        if($type == 'user')
            $url = 'https://api.vk.com/method/users.get?user_ids=' . $screen_name .'&v=5.103';
        else
            $url = 'https://api.vk.com/method/groups.getById?group_id=' . $screen_name .'&v=5.103';
        ...
        try
        {
            ...
            if ($data->code == 200)
            {
                ...
                {
                    $id = $type == 'user' ? $data['response'][0]['id'] : '-' . $data['response'][0]['id']; //if page or group, add a - (negative) prefix
                }

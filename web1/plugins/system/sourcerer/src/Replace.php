...
class Replace
{
	...
	private static function loadScripts($data)
	{
		...
		self::loadMediaFile($file, 'js');
	}

	private static function loadMediaFile($file, $type)
	{
		if (empty($file))
		{
			return;
	...
	private static function getPhpFileCodeByType($file, $type)
	{
		...
		foreach ($files as $file)
		{
			...
			if (strpos($type, 'include') !== false && ! JFile::exists(JPATH_SITE . $params->include_path . $file))
			{
				continue;
			}

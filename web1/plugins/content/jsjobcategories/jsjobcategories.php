...
class plgContentJSJobcategories extends JPlugin
{
	...
	public function onPrepareContent( &$row, &$params, $page=0 )  {
		...
		$slidingleftwidth= $this->params->get('slidingleftwidth','0'); // 0 = left  , 1=up
		//sce
		...
		$result = JSModel::getJSModel('category')->getJobCategories($theme);
		$categories = $result[0];
		$dateformat = $result[2];	
		/** sce */
        $color = array();
	...
	if ($categories) { 
		...
		foreach ($categories as $category) {
			...
			if ($printrecord == 1){	
				$lnks = 'index.php?option=com_jsjobs&c=job&view=job&layout=jobs&cat='. $category->aliasid .'&Itemid='.$itemid; 
				$lnks = JRoute::_($lnks);
				$contents .=  '<span id="jsjobs_modulelist_databar" class="module-list visible-all" style="width:'.$columnwidth.'%;"><span id="themeanchor"><a class="anchor" href="'.$lnks.'" >'.$category->cat_title;

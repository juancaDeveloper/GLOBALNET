...
{   // for joomla 1.5
	...
	public function onPrepareContent( &$row, &$params, $page=0 )  {
       ...
       public function onContentPrepare( $context, &$row, &$params, $page=0 ){
		...
		$slidingdirection= $this->params->get('slidingdirection',0); // 0 = left  , 1=up
		$slidingleftwidth= $this->params->get('slidingleftwidth','768'); // 0 = left  , 1=up
		//sce
		...
		$result = JSModel::getJSModel('states')->getJobsStates($showonlystatehavejobs,$theme,$noofrecord);
		$states = $result[0];
        $contentstitle = '';
        $contents = '';
if ($states) { 
		...
		foreach ($states as $state) {
			$lnks = 'index.php?option=com_jsjobs&c=job&view=job&layout=listnewestjobs&state='. $state->stateid .'&lt=1&Itemid='.$itemid; 
			$lnks = JRoute::_($lnks);
			$contents .=  '<span id="jsjobs_modulelist_databar" class="module-list visible-all" style="width:'.$columnwidth.'%;"><span id="themeanchor"><a class="anchor" href="'.$lnks.'" >'.$state->statename;

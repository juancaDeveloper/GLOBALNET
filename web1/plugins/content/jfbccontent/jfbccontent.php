...
class plgContentJFBCContent extends JPlugin
{
    ...
    private function getLikeBtn($article, $infix = 'article')
    {
        ...
        if ($showLinkedIn || $showTwitter || $showPinterest)
            $likeExtraHTML = JFBConnectUtilities::getExtraShareButtons($url, $buttonStyle, $size, false, false, $showTwitter, $this->renderKeyString, $showLinkedIn, '50', $showPinterest, $pinImage, $pinText);
    ...
    private function getPinterestText($article)
    {
        ...
        $text = current(array_filter($text));
        $text = preg_replace('/\s+/', ' ', $text);
        return $text; // return first non-null text in the array
    }

...
class plgContentJSJobsCities extends JPlugin
{
	...
	public function onPrepareContent( &$row, &$params, $page=0 )  {
       ...
       public function onContentPrepare( $context, &$row, &$params, $page=0 ){
		...
		//sce
			...
			$result = JSModel::getJSModel('cities')->getJobsCity($showonlycityhavejobs,$theme,$noofrecord);
			$cities = $result[0];
			$dateformat = $result[2];
	...
	if ($cities) { 
		...
		foreach ($cities as $city) {
			$lnks = 'index.php?option=com_jsjobs&c=job&view=job&layout=jobs&city='. $city->cityid .'&lt=1&Itemid='.$itemid; 
			$lnks = JRoute::_($lnks);

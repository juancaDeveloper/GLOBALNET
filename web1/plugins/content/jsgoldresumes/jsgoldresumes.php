 ...
 * Created on:	Aug 25, 2010
  ^
  + Project: 		JS Jobs
 * File Name:	Pplugin/jsgoldresume.php
  ^
...
class plgContentJSGoldResumes extends JPlugin {    // for joomla 1.5
    ...
    protected function _load($position, $style = -2) {
        ...
        //sce
        $resumeresult = JSModel::getJSModel('resume')->getGoldResumes($result['noofresumes'], true);
        ...
        if(isset($result['color'][1]) && !empty($result['color'][1])){
            $jobs_html .= 'div.'.$classname.' div#jsjobs_module span#themeanchor a{color:'.$result['color'][1].';}';
        }
        if(isset($result['color'][2]) && !empty($result['color'][2])){
            $jobs_html .= 'div.'.$classname.' div#jsjobs_module{background:'.$result['color'][2].';}';
            $jobs_html .= 'div.'.$classname.' div#jsjobs_modulelist_databar{background:'.$result['color'][2].';}';
            $jobs_html .= 'div.'.$classname.' div#jsjobs_modulelist_titlebar{background:'.$result['color'][2].';}';
        }
        if(isset($result['color'][3]) && !empty($result['color'][3])){
            $jobs_html .= 'div.'.$classname.' div#jsjobs_module{border: 1px solid '.$result['color'][3].';}';
            $jobs_html .= 'div.'.$classname.' div#jsjobs_modulelist_titlebar{border: 1px solid '.$result['color'][3].';}';
            $jobs_html .= 'div.'.$classname.' div#jsjobs_modulelist_databar{border: 1px solid '.$result['color'][3].';}';
        } 
        if(isset($result['color'][4]) && !empty($result['color'][4])) {
            $jobs_html .= 'div#jsjobs_module_wrapper.'.$classname.' div#jsjobs_module_wrap div#jsjobs_module_data_fieldwrapper span#jsjobs_module_data_fieldtitle{color:'.$result['color'][4].';}';
            $jobs_html .= 'div.'.$classname.' div#jsjobs_modulelist_databar{color:'.$result['color'][4].';}';
            $jobs_html .= 'div.'.$classname.' div#jsjobs_modulelist_titlebar span#jsjobs_modulelist_titlebar{color:'.$result['color'][4].';}';
        } 
        if(isset($result['color'][5]) && !empty($result['color'][5])) {
            $jobs_html .= 'div#jsjobs_module_wrapper.'.$classname.' div#jsjobs_module_wrap div#jsjobs_module_data_fieldwrapper span#jsjobs_module_data_fieldvalue{color:'.$result['color'][5].';}';
        } 
        if(isset($result['color'][6]) && !empty($result['color'][6])) {
            $jobs_html .= 'div.'.$classname.'  div#jsjobs_module span#jsjobs_module_heading {border-bottom: 1px solid '.$result['color'][6].';}';
        }
        $jobs_html .= '</style>';
        return $jobs_html;   
    }
